...
class UIComponents:
    ...
    def render_person_metrics(
        person_name: str, financials: PersonFinancials, show_bonus: bool = False
    ):
        """
        ...
        if show_bonus and financials.bonus > 0:
            st.caption(
                f"(Base: {financials.salary:,.0f} + Bonus: {financials.bonus:,.0f})"
            )
        st.write(
            f"Shared: CHF {financials.shared_expenses:,.0f} ({financials.split_rate:.0f}%)"
        )
        st.write(f"Personal: CHF {financials.personal_expenses:,.0f}")
        st.write(
            f"**Savings: CHF {financials.net_savings:,.0f}** ({financials.savings_rate:.0f}%)"
        )
    ...
    @staticmethod
    def render_totals_column(person_name: str, totals: Dict[str, float]):
        """
        ...
        savings_rate = (
            (totals["net_savings"] / totals["net_salary"] * 100)
    ...
    def render_combined_totals(
        guillem_totals: Dict[str, float], vero_totals: Dict[str, float]
    ):
        """
        ...
        combined_shared = (
            guillem_totals["shared_expenses"] + vero_totals["shared_expenses"]
        )
        combined_personal = (
            guillem_totals["personal_expenses"] + vero_totals["personal_expenses"]
        )
    ...
    @staticmethod
    def render_monthly_summaries_section(
        period_financials_list: List[PeriodFinancials],
    ):
        """
