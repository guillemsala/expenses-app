...
class FinancialCalculator:
    ...
    def calculate_period_financials(
        ...
        guillem_salary = (
            period_data["guillem_salary"].iloc[0] if len(period_data) > 0 else 0
        )
        vero_salary = period_data["vero_salary"].iloc[0] if len(period_data) > 0 else 0
        guillem_bonus = (
            period_data["guillem_bonus"].iloc[0]
            if "guillem_bonus" in period_data.columns and len(period_data) > 0
            else 0
        )
        vero_bonus = (
            period_data["vero_bonus"].iloc[0]
            if "vero_bonus" in period_data.columns and len(period_data) > 0
            else 0
        )
        ...
        guillem_split_rate = (
            (guillem_shared / total_shared * 100) if total_shared > 0 else 0
        )
        ...
        savings_rate_vero = (
            (net_savings_vero / vero_total_income * 100) if vero_total_income > 0 else 0
        )
