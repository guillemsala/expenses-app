...
class FinancialCalculator:
    ...
    def calculate_period_financials(
        period_data: pd.DataFrame, period: str
    ) -> PeriodFinancials:
        """
        ...
        """
        # Extract incomes and bonuses
        (
            guillem_salary,
            vero_salary,
            guillem_bonus,
            vero_bonus,
            guillem_total_income,
            vero_total_income,
        ) = FinancialCalculator._extract_incomes(period_data)

        # Compute shared and personal expense aggregates
        (
            guillem_shared,
            vero_shared,
            total_shared,
            personal_guillem,
            personal_vero,
        ) = FinancialCalculator._sum_shared_and_personal(period_data)

        # Compute totals, rates and combined metrics
        (
            guillem_split_rate,
            vero_split_rate,
            total_expenses_guillem,
            total_expenses_vero,
            net_savings_guillem,
            net_savings_vero,
            savings_rate_guillem,
            savings_rate_vero,
            combined_income,
            combined_savings,
            combined_savings_rate,
        ) = FinancialCalculator._compute_rates_and_totals(
            guillem_shared,
            vero_shared,
            total_shared,
            personal_guillem,
    ...
    @staticmethod
    def _extract_incomes(period_data: pd.DataFrame):
        """Helper to extract salaries and bonuses and compute totals."""
        if len(period_data) == 0:
            return 0, 0, 0, 0, 0, 0

        guillem_salary = period_data["guillem_salary"].iloc[0]
        vero_salary = period_data["vero_salary"].iloc[0]
        guillem_bonus = (
            period_data["guillem_bonus"].iloc[0]
            if "guillem_bonus" in period_data.columns
            else 0
        )
        vero_bonus = (
            period_data["vero_bonus"].iloc[0]
            if "vero_bonus" in period_data.columns
            else 0
        )
    ...
    @staticmethod
    def _sum_shared_and_personal(period_data: pd.DataFrame):
        """Helper to sum shared and personal expense components."""
        if len(period_data) == 0:
            return 0, 0, 0, 0, 0
    ...
    def _compute_rates_and_totals(
        ...
        """Helper computing derived totals and rates."""
        ...
        savings_rate_guillem = (
            (net_savings_guillem / guillem_total_income * 100)
            if guillem_total_income > 0
            else 0
        )
        savings_rate_vero = (
            (net_savings_vero / vero_total_income * 100)
            if vero_total_income > 0
            else 0
        )
        ...
        combined_savings_rate = (
            (combined_savings / combined_income * 100) if combined_income > 0 else 0
        )
