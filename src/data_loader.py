...
class DataLoader:
    ...
    def _validate_schema(self, df: pd.DataFrame) -> bool:
        ...
        invalid_splits = df[~df["split_type"].isin(self.VALID_SPLIT_TYPES)][
            "split_type"
        ].unique()
        if len(invalid_splits) > 0:
            st.warning(
                f"⚠️ Invalid split_type values found: {', '.join(invalid_splits)}"
            )
    ...
    def _preprocess(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["for_month"] = pd.Categorical(
            df["for_month"], categories=self.MONTH_ORDER, ordered=True
        )
        ...
        df["period_display"] = (
            df["for_month"].astype(str) + " " + df["for_year"].astype(str)
        )
    ...
    def get_available_periods(df: pd.DataFrame) -> list:
        ...
        """
        periods = (
            df.groupby(["for_year", "for_month", "period_display"]).size().reset_index()
        )
